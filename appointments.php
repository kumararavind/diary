...
<html lang="zxx">
   ...
   $id=$_GET['uid'];
	$query=mysqli_query($con,"SELECT * from appointments where uid='$id'")or die(mysqli_error($con));
	 ...
	 <form action="" method="POST">
	 	<div class="row">
	 		<div class="col-md-6 form-group">
	 		 	...
	 		 	<input type="text" name="ap_on" class="form-control" placeholder="Enter Appointment Date" value="<?php echo $row['ap_on']; ?>">
	 		</div>	
	 	      <div class="col-md-6 form-group">
	 			<label>Appointment With</label>
	 			<input type="text" name="apwith" class="form-control" placeholder="Enter Appointment With" value="<?php echo $row['apwith']; ?>">
	 		</div>
	 	</div>
        <div class="row">
	 		<div class="col-md-6 form-group">
	 			<label>Appointment Reason</label>
	 			<input type="text" class="form-control" name="reason" placeholder="Enter Appointment Reason" value="<?php echo $row['reason']; ?>">
	 		</div>
	 		<div class="col-md-6 form-group">
	 			<label>Posted On</label>
	 			<input type="Date" name="postedon" class="form-control"  value="<?php echo $row['postedon']; ?>">
	 		</div>
	 	</div>
         <div class="row">
...
<?php include 'connection.php';
	...
	{
		$app_on=$_POST['ap_on'];
		$appwith=$_POST['apwith'];
		$reason=$_POST['reason'];
		$postedon=$_POST['postedon'];
		
		  $qry=mysqli_query($con,"UPDATE `appointments` SET `ap_on`='$app_on',`postedon`='$postedon',`reason`='$reason',`apwith`='$appwith' WHERE uid='$id'") or die(mysqli_error($con));
        if($qry==true)
			{
			...
			{
				echo '<script>alert("Update Failed");window.location="appointments.php";</script>';
			}
		}
?>
